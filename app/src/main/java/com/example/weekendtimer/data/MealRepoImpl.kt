...
// We are defining a class called 'MealRepoImpl'. This class will be responsible for getting data from the API.
...
class MealRepoImpl @Inject constructor(private val service: APIService) : MealRepo {
    ...
    override suspend fun getMealInCategory(category: String): List<Meal> {
        val mealDTOs: List<MealDTO> = service.getMealFromCategory(category).meals
        return mealDTOs.map {
            Meal(
                idMeal = it.idMeal,
                strMeal = it.strMeal,
